...
describe('Dashboard Access', function () {
    ...
    it('returns personal-only scoped metrics for admin users', function () {
        ...
        $response->assertInertia(fn ($page) => $page
            ...
            ->where('metricsByScope.personal.total_blog_post_views', 0)
            ->where('metricsByScope.personal.average_views_per_blog_post', 0)
            ...
            ->has('metricsByScope.personal.views_30d', 30)
            ->has('metricsByScope.personal.high_value_metrics.top_authors_by_published_posts_30d', 2)
            ->where('metricsByScope.personal.high_value_metrics.top_authors_by_published_posts_30d', fn ($rows) => collect($rows)->pluck('id')->contains($admin->id))
    ...
    it('returns both personal and global scoped metrics for master admins', function () {
        ...
        $response->assertInertia(fn ($page) => $page
            ...
            ->has('metricsByScope.personal.views_30d', 30)
            ->has('metricsByScope.personal.high_value_metrics.top_authors_by_published_posts_30d', 2)
            ->where('metricsByScope.personal.high_value_metrics.top_authors_by_published_posts_30d', fn ($rows) => collect($rows)->pluck('id')->contains($masterAdmin->id))
            ->has('metricsByScope.personal.high_value_metrics.posts_published_30d', 30)
            ->has('metricsByScope.personal.high_value_metrics.comments_created_30d', 30)
            ->has('metricsByScope.personal.high_value_metrics.likes_created_30d', 30)
            ->has('metricsByScope.personal.high_value_metrics.follower_growth_30d', 30)
