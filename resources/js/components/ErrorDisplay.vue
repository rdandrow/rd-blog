...
<template>
    <div v-if="error" :class="['rounded-lg border p-4', getBgClass(type)]">
        <div class="flex items-center gap-2">
            <svg
                :class="['h-5 w-5 flex-shrink-0', getIconClass(type)]"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
            >
            ...
            <span :class="['flex-1 font-medium', getTextClass(type)]">
                {{ error }}
            </span>
            ...
            <button
                ...
                @click="emit('dismiss')"
                :class="[
                    'ml-2 transition-opacity hover:opacity-70',
                    getIconClass(type),
                ]"
                aria-label="Dismiss"
            >
