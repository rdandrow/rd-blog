...
const {
    ...
    `blog-post-edit-draft-${props.post.id}`,
    formDataForAutoSave,
    30000,
);
...
const submit = async () => {
    try {
        ...
        try {
            ...
            const hasExcerpt =
                form.excerpt && String(form.excerpt).trim().length > 0;
            const hasContent =
                form.content && String(form.content).trim().length > 0;

            // If we actually have all required fields, log this as a validation bug but continue
            if (hasTitle && hasExcerpt && hasContent) {
                console.warn(
                    'Client-side validation failed but fields appear to be valid. Continuing with server validation.',
                );
            } else {
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="max-w-form mx-auto">
            ...
            <div
                v-if="error"
                class="mb-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-800 dark:border-amber-800 dark:bg-amber-950 dark:text-amber-200"
            >
            ...
            >
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        >
                            We found an auto-saved draft from your previous
                            editing session. Would you like to restore it?
                        </p>
                        <div class="mt-3 flex gap-3">
                            ...
                            <button
                                type="button"
                                @click="dismissDraft"
                                class="rounded-md border border-blue-200 bg-white px-4 py-2 text-sm text-blue-600 transition-colors hover:bg-blue-50 dark:border-blue-700 dark:bg-gray-800 dark:text-blue-400 dark:hover:bg-gray-700"
                            >
                                Dismiss
                            </button>
                        </div>
                ...
                >
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            required
                            rows="3"
                            class="w-full resize-y rounded-md border border-border bg-background px-3 py-2 text-foreground placeholder:text-muted-foreground focus:border-transparent focus:ring-2 focus:ring-ring focus:outline-none"
                            placeholder="Brief description of your post..."
                        />
                        <p class="mt-1 text-sm text-muted-foreground">
                            A short summary that appears in post listings and
                    ...
                    <div>
                        ...
                        <div v-if="imageInputType === 'url'">
                            <input
                                ...
                                :value="
                                    isExternalUrl(form.featured_image)
                                        ? form.featured_image
                                        : ''
                                "
                        ...
                        <div v-if="imageInputType === 'file'">
                            <input
                                ...
                                class="w-full rounded-md border border-border bg-background px-3 py-2 text-foreground file:mr-4 file:rounded file:border-0 file:bg-muted file:px-4 file:py-2 file:text-sm file:font-semibold file:text-muted-foreground hover:file:bg-muted/80"
                            />
                            <p class="mt-1 text-sm text-muted-foreground">
                                Upload an image from your computer (JPG, PNG,
                                GIF, WebP).
                            </p>
                        </div>
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    ...
                                    <button
                                        ...
                                        class="mt-1 text-sm text-destructive hover:text-destructive/80"
                                    >
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex gap-2">
                                <input
                                    ...
                                    class="flex-1 rounded-md border border-border bg-background px-3 py-2 text-foreground placeholder:text-muted-foreground focus:border-transparent focus:ring-2 focus:ring-ring focus:outline-none"
                ...
                >
                    ...
                    <div class="flex items-center gap-3">
                        <input
                            ...
                            type="checkbox"
                            class="h-4 w-4 rounded border border-border focus:ring-2 focus:ring-ring"
                        />
                        <label
                            for="is_featured"
                    ...
                    <div v-if="form.is_published" class="ml-7">
                        ...
                        <input
                            ...
                            type="datetime-local"
                            class="rounded-md border border-border bg-background px-3 py-2 text-foreground focus:border-transparent focus:ring-2 focus:ring-ring focus:outline-none"
                        />
                        <p class="mt-1 text-sm text-muted-foreground">
                            Leave blank to use current publish date, or set a
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <button
                            type="button"
                            @click="
                                $inertia.visit(`/admin/blog-posts/${post.id}`)
                            "
                    ...
                    <button
                        ...
                        class="inline-flex items-center rounded-md bg-primary px-6 py-2 text-primary-foreground transition-colors hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                        <svg
